...
export default class Uploader {
  ...
  uploadSelectedFile({ onPreview }) {
    ...
    const preparePreview = function (res) {
      onPreview(res.file.url);
    };
    ...
    if (
      ...
      typeof this.config.uploader.selectFiles === 'function' &&
      typeof this.config.uploader.uploadByFile === 'function'
    ) {
      upload = this.config.uploader
        .selectFiles({ accept: this.config.types })
        .then(res => {
          preparePreview(res);

          const customUpload = this.config.uploader.uploadByFile(res);
